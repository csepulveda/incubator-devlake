...
var CalculateBusinessCycleTimeMeta = plugin.SubTaskMeta{
	Name:             "calculateBusinessCycleTime",
	EntryPoint:       CalculateBusinessCycleTime,
	EnabledByDefault: false, // Disabled by default - run manually after DORA completes to ensure commits_diffs exists
	Description:      "Calculate full business cycle time from ClickUp task to deployment (requires DORA/refdiff to run first)",
	DomainTypes:      []string{plugin.DOMAIN_TYPE_TICKET, plugin.DOMAIN_TYPE_CICD},
	Dependencies:     []*plugin.SubTaskMeta{}, // Requires commits_diffs from DORA/refdiff to be generated first
}

func CalculateBusinessCycleTime(taskCtx plugin.SubTaskContext) errors.Error {
	db := taskCtx.GetDal()
	data := taskCtx.GetData().(*ClickUpTaskData)

	taskCtx.GetLogger().Info("Starting ClickUp business cycle time calculation for connection: %d", data.Options.ConnectionId)

	// Get all ClickUp tasks with PR numbers from this connection
	cursor, err := db.Cursor(
		dal.From(&models.ClickUpTask{}),
		dal.Where("connection_id = ? AND git_hub_pr_number > 0", data.Options.ConnectionId),
	)
	if err != nil {
		return err
	}
